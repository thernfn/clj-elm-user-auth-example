...
{-| By modifying the settings type, our layout will ask each page to provide
those two fields before it can be rendered. In this example our sidebar layout
will have a title at the top of each page. It will also need the current user,
for display in the sidebar itself. Having the route value will let us higlight
the current page so our users know where they are.
-}
type alias Settings =
    { title : String
    , user : Auth.User
...
{-| For our sidebar we'll want settings and route available in our view function,
so that the settings.title and settings.user values are available when we render
