...
{-
   Our sign-in form will involved two fields, "Email" and "Password". We need a way
   to track the current value in the <input> element. We use two String fields in
   our sign-in page's Model. Additionally, we'll want to disable the "Sign in"
   button when the user clicks it. This will let them know that a sign-in request
   is in progress and their form submission was successful.
-}
-- INIT
...
-- UPDATE
{-
   We'll need to define all the ways a user could interact with our Sign-in page.
   Let's add a UserUpdatedInput message for handling when our <input> fields receive
   new values.
-}
